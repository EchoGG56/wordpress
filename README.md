...
| 表名           | 说明      | 关键字段                                                                                     |
| ------------ | ------- | ---------------------------------------------------------------------------------------- |
| `users`      | 用户表     | id, username, email, password\_hash, avatar, created\_at                                 |
| `posts`      | 文章表     | id, title, content, excerpt, cover\_image, author\_id, category\_id, status, created\_at |
| `categories` | 分类表     | id, name, slug                                                                           |
| `tags`       | 标签表     | id, name                                                                                 |
| `post_tags`  | 文章-标签关联 | post\_id, tag\_id (联合主键)                                                                 |
...
| 方法     | 路径                        | 说明     | 认证      |
| ------ | ------------------------- | ------ | ------- |
| GET    | `/posts/:postId/comments` | 获取文章评论 | 否       |
| POST   | `/posts/:postId/comments` | 创建评论   | 是       |
| DELETE | `/comments/:id`           | 删除评论   | 是（作者本人） |
...
- **文章列表**：Grid 卡片布局，支持分类筛选（全部/技术/生活/随笔）
- **文章详情**：基于 Hash 路由 (`#post-{id}`)，底部评论区
- **评论系统**：支持发表评论、嵌套回复、删除
...
- [ ] 无分页 UI 组件
- [x] 评论系统（已实现，支持嵌套回复）
- [x] 分类筛选（已实现，全部/技术/生活/随笔）
- [x] 注册校验（邮箱格式 + 密码至少8位含大小写和特殊字符）
...
```
3ea8fce feat: 添加注册校验和分类筛选功能
19c8983 feat: 添加文章评论功能
eda52bf docs: 添加项目落地文档 README.md
